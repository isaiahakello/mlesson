...
foreach($details as $detail){
  ...
  switch($service){
    ...
    case 'bvnj58t4': //world records
     ...
     print_r($results);
     $action = end($results);
     $query = mysql_query("UPDATE subscribers SET action = '$action',status='1' WHERE id = '$id'");
}
function build_url($base_url,$username,$password,$message,$msisdn,$id){
    $query_string ='username='.urlencode($username).'&text='.urlencode($message).'&msisdn='.urlencode($msisdn).'&subsaction=subscribe&cost=0&clientref='.$id;
